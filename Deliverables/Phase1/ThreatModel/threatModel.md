...
| 2  | The **eMovie Shop Backend**, built in Java, depends on the Java runtime (JDK) and an ecosystem of external libraries (e.g., Spring Boot, JPA/Hibernate). These must be actively maintained to avoid outdated packages introducing vulnerabilities.                                                                               |
...
| 2.1   | Availability of the Backend               | The Java backend must be online and responsive 24/7 to process API requests.                                                                                                                                                     | (7) System Administrator                                                            |
...
| 3.5   | Receipt Files                             | Server-side `.txt` receipt files generated during the purchase workflow (UC3). Filenames are derived from user-supplied `receiptName`. Sensitive to path traversal, command injection via filename, and unauthorized overwrite.       | (4) Customer (triggers creation), (7) System Administrator (filesystem access)      |
...
| 5.1   | Backend REST API                          | Java application processing requests and enforcing RBAC.                                                                                                                                                                          | (3) Authenticated User                                                              |
...
- Users authenticate directly with **Auth0** (the external Identity Provider), which validates credentials and issues a signed JWT. The backend never receives user passwords, it only validates the JWT presented in API requests by fetching Auth0's JWKS public keys.
   * The interaction between the backend and Auth0 is limited to JWKS key retrieval for token signature verification.
...
| U2     | Spoofing                   | An attacker performs brute-force or credential stuffing against Auth0's login page, attempting to obtain valid credentials.                                     |
...
| U5     | Denial of Service          | An attacker floods the backend with repeated requests to resource-intensive endpoints (e.g., `GET /movies`, `POST /orders`), causing service degradation.       |
...
| B9     | Tampering                | Path traversal via user-supplied filename (e.g., `receiptName` in UC3) allows an attacker to write files outside the intended sandboxed directory if the filename is not properly sanitized.  |
| B10    | Tampering                | OS command injection via user-supplied input (e.g., `receiptName`) if the value is ever interpolated into a shell command rather than handled through direct file I/O APIs.                   |
| B11    | Denial of Service        | Filesystem resource exhaustion through mass creation of receipt files via automated order submissions, potentially filling disk space or degrading I/O performance.                            |

**Backend -> Auth0 (Backend / External Authentication Service Boundary)**

| ID     | Category                 | Description                                                                                                                                                                           |
|--------|--------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| A1     | Spoofing                 | An attacker intercepts the token validation request between the backend and Auth0's JWKS endpoint (e.g., via a compromised network path) and injects a forged key set.                |
| A2     | Spoofing                 | Auth0 tenant misconfiguration (e.g., wrong audience, disabled connections) causes authentication failures that an attacker could exploit to force fallback mechanisms.                 |
| A3     | Tampering                | The Auth0 `audience` or `issuer` values are tampered with in the backend environment, causing the backend to accept tokens from an attacker-controlled identity provider.              |
| A4     | Information Disclosure   | Auth0 tenant configuration details (e.g., client metadata, user attributes) are exposed if the Auth0 Management API is overly permissive or credentials are leaked.                   |
...
* UC2: [Use/Abuse Case UC2](../UseCases/UC2_BrowseAvailableMovies/README.MD)
...
Additionally, the receipt file creation workflow (UC3) introduces filesystem-specific injection risks: threat B9 addresses path traversal via user-supplied filenames, and B10 covers OS command injection if user input is ever interpolated into shell commands, both extending **A03** coverage beyond traditional SQL injection.
...
* CWE-22: Improper Limitation of a Pathname to a Restricted Directory ('Path Traversal') maps to threat B9, where user-supplied filenames in the receipt generation workflow could allow writing files outside the intended sandboxed directory if not properly sanitized.
* CWE-78: Improper Neutralization of Special Elements used in an OS Command ('OS Command Injection') maps to threat B10, covering the risk of shell command execution through unsanitized user input in file operations.
...
* CWE-287: Improper Authentication maps to threats U2, B1, and A1, covering brute-force credential attacks, weak JWT validation, and JWKS interception during token verification. 
...
To assess and prioritize the identified threats in eMovie Shop, we apply a **Qualitative Risk Model** inspired by the [OWASP Risk Rating Methodology](https://owasp.org/www-community/OWASP_Risk_Rating_Methodology), based on two key dimensions:
...
| B9    | Path traversal via user-supplied filename            | Tampering                | 4      | 3             | 12     |
| B10   | OS command injection via user-supplied input          | Tampering                | 5      | 2             | 10     |
| B11   | Filesystem exhaustion via mass file creation          | Denial of Service        | 3      | 3             | 9      |

**Backend → Auth0 (External Authentication Service Boundary)**

| ID    | Threat                                               | Category                | Cost  | Probability   | Risk   |
|-------|------------------------------------------------------|-------------------------|-------|---------------|--------|
| A1    | JWKS request intercepted, forged key set injected    | Spoofing                | 4     | 2             | 8      |
| A2    | Auth0 tenant misconfiguration exploited              | Spoofing                | 4     | 2             | 8      |
| A3    | Auth0 audience/issuer tampered to accept rogue tokens | Tampering              | 5     | 1             | 5      |
| A4    | Auth0 tenant config exposed via Management API leak  | Information Disclosure  | 4     | 2             | 8      |
...
| B9              | Path traversal via user-supplied filename in receipt generation   | Sanitize filenames with a strict allow-list (alphanumeric, hyphens, underscores); Strip path separators and null bytes; Resolve the final path and verify it remains inside the sandboxed directory before writing.                           |
| B10             | OS command injection via user-supplied input                      | Never pass user-supplied values to shell commands; Use only direct file I/O APIs (e.g., `java.nio.file`); The allow-list sanitization provides defense-in-depth by rejecting shell metacharacters.                                           |
| B11             | Filesystem resource exhaustion via mass receipt file creation     | Apply rate limiting on order creation endpoints; Implement periodic cleanup of old receipt files; Monitor disk usage and alert on abnormal growth.                                                                                            |
| A1              | JWKS request intercepted, forged key set injected                | Enforce TLS on all backend-to-Auth0 communication; Pin or validate the Auth0 JWKS endpoint certificate; Cache JWKS with a bounded TTL to reduce exposure.                                                                                   |
| A2              | Auth0 tenant misconfiguration exploited by attacker              | Regularly audit Auth0 tenant settings (allowed audiences, connection policies, enabled grants); Enable Auth0 anomaly detection and alert on configuration changes.                                                                            |
| A3              | Auth0 audience/issuer tampered to accept rogue tokens            | Store Auth0 configuration exclusively in server-side environment variables; Restrict environment access to authorized personnel; Verify the Auth0 token issuer (`iss`) on every JWT validation.                                              |
| A4              | Auth0 tenant config exposed via Management API leak              | Restrict Auth0 Management API access to authorized personnel only; Use scoped tokens with minimum privileges; Monitor Auth0 logs for unauthorized Management API calls.                                                                      |
...
| **Countermeasure(s)**                                                                                | **ASVS Reference(s)**          | **Test Planning**                                                                                                                                                                                          |
|------------------------------------------------------------------------------------------------------|--------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Enforce server-side RBAC via `RoleGuard` on every protected route                                    | V8.2.1, V8.3.1                | Using Postman, send requests to support/admin-only endpoints (e.g., `GET /refunds`, `PATCH /movies/:id`) with a Customer-role JWT and verify a `403 Forbidden` response is returned.                       |
| Validate JWT signature, issuer, audience, and expiry on every request                                | V9.1.1, V9.2.1                | Tamper with a valid JWT (modify payload claims or signature) and submit it to a protected endpoint; verify the backend rejects it with `401 Unauthorized`.                                                 |
| Use RS256 or HS512; strictly validate `iss`, `aud`, and `exp` claims                                 | V9.1.1, V9.2.2, V9.2.3       | Submit an expired token, a token with an incorrect `aud`, and a token signed with a different key; confirm all are rejected with appropriate error responses.                                              |
| Enforce rate limiting and account lockout on Auth0's login page                                      | V2.4.1, V6.3.1                | Simulate repeated failed login attempts against Auth0 and verify that Auth0's attack protection triggers lockout after the defined threshold.                                                              |
| Enable Auth0 Attack Protection (brute-force and breached password detection)                         | V6.3.1, V6.8.2                | In the Auth0 dashboard, verify that brute-force protection is active; trigger the threshold and confirm Auth0 blocks further attempts and logs the event.                                                  |
| Enforce TLS end-to-end; apply HSTS; reject plain HTTP requests                                       | V12.1.2, V12.3.1              | Attempt to call the API over plain HTTP and verify it is either rejected or redirected; inspect response headers with `curl -I` to confirm `Strict-Transport-Security` is present.                         |
| Store `client_secret` and Auth0 config in environment variables; integrate secrets scanning in CI/CD | V13.3.1, V14.2.1              | Inspect the source code repository and build artifacts to confirm no credentials are hardcoded; run a secrets scanning tool (e.g., GitGuardian or `git-secrets`) against the commit history.               |
| Sanitize and validate all user-supplied input on the backend                                         | V1.1.1, V2.2.1, V2.2.2       | Inject malformed or oversized payloads into fields such as refund reason or movie description and verify the backend returns a validation error without persisting or reflecting the input.                |
| Use ORM with parameterized queries; run static analysis for injection patterns                       | V1.2.1, V1.2.2                | Attempt SQL injection via query parameters and request body fields (e.g., `' OR 1=1 --`); verify the ORM prevents execution; run SonarQube or Semgrep and confirm no injection-prone patterns are flagged. |
| Disable wildcard CORS; restrict to explicitly trusted origins                                        | V4.3.1, V13.2.1               | Send API requests from an untrusted origin using a modified `Origin` header and verify the server returns a CORS error; confirm no wildcard `Access-Control-Allow-Origin` header is present.               |
| Return generic error messages in production; use centralized error handling                          | V16.5.1                       | Use OWASP ZAP or Postman to trigger server errors (e.g., malformed requests, invalid IDs) and verify that responses contain no stack traces, file paths, or internal identifiers.                          |
| Implement structured audit logging for all sensitive actions                                         | V16.2.1, V16.3.1, V16.3.2    | Trigger sensitive operations (login, refund approval/rejection, role change, catalog edit) and inspect the audit log store to verify each entry includes timestamp, actor ID, source IP, and outcome.      |
| Apply rate limiting on all endpoints; enforce per-IP and per-user throttling                         | V2.4.1, V4.2.1                | Flood a resource-intensive endpoint (e.g., `GET /movies`, `POST /refunds`) with rapid successive requests and verify `429` responses are returned and normal traffic recovers after the window expires.    |
| Enforce mandatory pagination on all list endpoints; apply ORM query limits                           | V2.3.4                        | Call list endpoints (e.g., `GET /movies`, `GET /refunds`) without pagination parameters and verify the backend enforces a maximum result limit; confirm no unbounded query reaches the database.           |
| Apply least-privilege DB accounts; restrict permissions per service role                             | V13.3.1                       | Attempt write operations using a read-only database account (via direct DB client or backend inspection) and verify they are rejected; review DB user grants to confirm minimum privilege is applied.      |
| Apply `WHERE user_id = ?` ownership filters on all user-scoped queries                               | V8.3.3                        | Authenticate as Customer A and attempt to retrieve or modify records belonging to Customer B (e.g., orders, refunds) by manipulating IDs in the request; verify the backend returns `403` or `404`.        |
| Enforce HTTPS for all backend-to-Auth0 communication; validate Auth0 endpoint certificate            | V12.1.2, V12.3.1              | Inspect outbound requests from the backend to Auth0 (e.g., via proxy or integration test logs) and confirm all calls use HTTPS with a valid certificate; verify no fallback to HTTP exists.                |
| Monitor Auth0 logs for token issuance from unexpected sources                                        | V16.3.1, V10.2.1              | In the Auth0 dashboard, review the log stream after normal operation and after a simulated `client_secret` leak scenario; verify alerts are triggered for anomalous token issuance patterns.               |
| Enforce application-level ownership checks before any write operation                                | V8.3.1, V8.3.3                | Submit PATCH/DELETE requests targeting records not owned by the authenticated user (varying IDs in the path); verify the service layer rejects the operation before it reaches the database.               |
| Log all CRUD operations through the backend with actor ID and affected record                        | V16.2.1, V16.3.2              | Perform a sequence of create, update, and delete operations and verify each is recorded in the audit log with the correct actor, record reference, timestamp, and operation type.                          |
| Sanitize filenames with allow-list; resolve final path inside sandboxed directory                    | V5.3.1, V5.3.2                | Submit receipt names containing path traversal sequences (e.g., `../../etc/passwd`, `..\Windows\`), null bytes, and shell metacharacters; verify the backend rejects or sanitizes them and the file is created only inside the receipts directory. |
| Use only direct file I/O APIs; never interpolate user input into shell commands                      | V1.2.1, V5.3.2                | Conduct static analysis (e.g., Semgrep) to confirm no user-supplied values are passed to `Runtime.exec()`, `ProcessBuilder`, or similar shell invocation APIs; review `ReceiptFileService` for direct I/O usage only.                              |
| Rate limit order creation; periodic cleanup of old receipt files; monitor disk usage                 | V2.4.1                         | Submit a burst of order creation requests and verify rate limiting triggers `429`; verify that a cleanup mechanism exists for old receipt files; confirm disk usage alerting is configured.                                                          |
...
| B9              | Path traversal via user-supplied filename in receipt generation.      |                    |                           |           X           |
| B10             | OS command injection via user-supplied input.                         |                    |                           |           X           |
| B11             | Filesystem exhaustion via mass receipt file creation.                 |                    |             X             |                       |
| A1              | JWKS request intercepted, forged key set injected.                   |                    |                           |           X           |
| A2              | Auth0 tenant misconfiguration exploited by attacker.                 |                    |             X             |                       |
| A3              | Auth0 audience/issuer tampered to accept rogue tokens.               |                    |             X             |                       |
| A4              | Auth0 tenant config exposed via Management API leak.                 |                    |             X             |                       |
