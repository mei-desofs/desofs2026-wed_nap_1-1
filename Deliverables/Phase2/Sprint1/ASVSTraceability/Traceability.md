...
- Status: `Partial`
- Implementation: token issuance and rotation are the IdP's responsibility. The application validates and rejects expired/invalid tokens but does not itself issue or revoke tokens; a server-side invalidation mechanism would require integration (token introspection/revocation list) with the IdP.
- Evidence / Tests:
	- `App/src/main/java/com/example/desofs/config/SecurityConfig.java` and `App/src/main/java/com/example/desofs/config/Auth0Config.java` — show JWT-based design and delegation of token lifecycle to IdP.
...
- Status: `Partial`
- Implementation: for a stateless JWT API the application rejects expired or invalid tokens; immediate revocation requires IdP support (revocation endpoint or short-lived tokens). The repo should document recommended revocation/rotation strategies for production deployments.
- Evidence / Tests:
	- `App/src/main/java/com/example/desofs/config/SecurityConfig.java` — JwtDecoder configuration.
	- Recommendation: integrate token introspection or short token lifetimes with refresh token handling in client/IdP.
